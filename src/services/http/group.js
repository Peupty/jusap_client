...
const acceptRequest = http => (id, users) =>
  http({
    method: 'post',
    url: `/api/group/${id}/forms`,
    data: users
...
const declineRequest = http => (id, users) =>
  http({
    method: 'post',
    url: `/api/group/${id}/forms/decline`,
    data: users
  })
...
export default http => ({
  ...
  acceptRequest: acceptRequest(http),
  declineRequest: declineRequest(http),
  leave: leave(http)
})
